...
def callback_rgb(msg):
    ...
    img_bin = cv2.inRange(img_hsv, numpy.array([40,120,35]), numpy.array([85, 255, 255]))
    

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(4,4))
    img_bin = cv2.morphologyEx(img_bin, cv2.MORPH_OPEN, kernel)	#We use it as a noise filter and we get a new Binary image

    idx = cv2.findNonZero(img_bin)

    [centroid_x, centroid_y, a, b] = cv2.mean(idx)
    cv2.circle(img_bgr, (int(centroid_x), int(centroid_y)), 40, [0, 255, 0], thickness=3)
