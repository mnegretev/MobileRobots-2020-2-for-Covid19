...
def move(msg):
    ...
    if(final_goal=='WINDOW'):
        goal_aux.pose.position.x=1.00
        goal_aux.pose.position.y=1.00
 	goal_pub.publish(goal_aux)
    elif(final_goal=='GARAGE'):
	goal_aux.pose.position.x=3.00
	goal_aux.pose.position.y=5.00
	goal_pub.publish(goal_aux)
    elif(final_goal=='DOOR'):
	goal_aux.pose.position.x=9.00
	goal_aux.pose.position.y=0.00
	goal_pub.publish(goal_aux)
    elif(final_goal=='BATHROOM'):
	goal_aux.pose.position.x=5.00
	goal_aux.pose.position.y=1.00
	goal_pub.publish(goal_aux)
